 ...
 * - Motors on M1 and M2 (feeders 0 and 1)
...
MotorState motors[2];  // For feeders 0 and 1

// Timer interrupt variables
volatile bool timer_interrupt_flag = false;
volatile unsigned long interrupt_counter = 0;
volatile unsigned long motor_target_times[2] = {0, 0};  // Target stop times for each motor
 
 // Beam Break Detection
bool beam_break_state[2] = {HIGH, HIGH};  // Previous states (HIGH = no break)
...
const unsigned long TTL_CHECK_INTERVAL = 2;  // Check every 2ms (faster for responsiveness)
 ...
 void setup() {
  ...
  setup_timer_interrupt();
   ...
   // Initialize Pins
   pinMode(BEAM_BREAK_PIN_0, INPUT_PULLUP);
   pinMode(BEAM_BREAK_PIN_1, INPUT_PULLUP);
 ...
 void activate_motor(int feeder_id, int duration_ms, int speed) {
  if (feeder_id < 0 || feeder_id > 1) {
    Serial.println("ERROR:Invalid feeder ID");
    return;
  }
...
ISR(TIMER1_COMPA_vect) {
  ...
  // Check motors every 1ms for precise timing
  for (int i = 0; i < 2; i++) {
...
void update_motors_interrupt() {
 ...
 void check_beam_breaks() {
   // Check feeder 0 beam break
   bool current_0 = digitalRead(BEAM_BREAK_PIN_0);
   if (beam_break_state[0] == HIGH && current_0 == LOW) {
     // Beam broken (HIGH to LOW transition)
     Serial.println("BEAM:0");
     // Blink LED to indicate detection
     digitalWrite(LED_PIN, LOW);
     delay(50);
     digitalWrite(LED_PIN, HIGH);
   }
   beam_break_state[0] = current_0;
   
   // Check feeder 1 beam break
   bool current_1 = digitalRead(BEAM_BREAK_PIN_1);
   if (beam_break_state[1] == HIGH && current_1 == LOW) {
     // Beam broken (HIGH to LOW transition)
     Serial.println("BEAM:1");
     // Blink LED to indicate detection
     digitalWrite(LED_PIN, LOW);
     delay(50);
     digitalWrite(LED_PIN, HIGH);
   }
   beam_break_state[1] = current_1;
 }
  ...
  * 
  * 2. Wiring:
  *    - Motors: Connect to M1 and M2 terminals on motor shield
  *    - Beam Break Sensors: 
